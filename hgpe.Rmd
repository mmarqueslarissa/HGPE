...
banco_intencao3 <- banco_intencao %>%
  mutate(data.x = lubridate::dmy(data))

## remover o ano da variavel data.x

# banco_intencao3$day <-lubridate::day(banco_intencao3$data.x)
# banco_intencao3$month<-lubridate::month(banco_intencao3$data.x)
# banco_intencao3$data_nova <- (paste0(as.character(banco_intencao3$day), as.character(banco_intencao3$month), format='%d-%m'))
# 
# d2$MthDD <- format(as.Date(d2$PickUpDate), "%m-%d")
# 
# banco_intencao3 <- banco_intencao3 %>% 
#   mutate(data_nova = lubridate::as_date(data_nova))
...
banco_intencao2 %>% 
  ...
  ggplot(aes(x = lubridate::as_date(data.x)))+
  ...
  scale_y_continuous(labels=function(x) paste0(x,"%"))+
  #scale_x_discrete(breaks = c('29/09/2020', '25/10/2020', '29/10/2020', '08/11/2020'))+
  # scale_x_date(date_breaks = "1 month", 
  #          limits = as.Date(c('2020-09-29', '2020-10-25', '2020-10-29', '2020-11-08'), format="%d/%m/%Y"),
  #          date_labels="%b-%Y" ) +
  # scale_x_date(date_breaks = "1 month", 
  #                labels= date_format("%b-%Y"),
  #                limits = as.Date(c('2020-09-29', '2020-10-25', '2020-10-29', '2020-11-08')))+
  scale_x_date(breaks = as.Date(c('2020-09-15','2020-09-29', '2020-10-25', '2020-10-29', '2020-11-08', '2020-11-25')),
               minor_breaks = as.Date(c('2020-09-15','2020-09-29', '2020-10-25', '2020-10-29', '2020-11-08', '2020-11-25')))+
  #(c('2020-09-29', '2020-10-29', '2020-11-08')
...
ggplot(aes(x=as.numeric(ano_eleitoral)))+ 
  ...
  scale_x_continuous(breaks = c(1982, 1988, 1992, 1996, 2000, 2004, 2008, 2012, 2016, 2020))+
