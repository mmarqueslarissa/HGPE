...
banco_intencao <- read_csv("banco_intencao.csv", 
     col_types = cols(Data = col_date(format = "%dd/%mm%/YYYY")))
...
ggplot(aes(x = Tempo.em.segundos, y = perc_votos, colour = Candidato, size =soma)) +
  ...
  # stat_regline_equation(formula = y ~ x)#+ #pq nao entra??
...
#   mutate(data.x = as.Date(data, "%YYYY-%mm-%dd"))
...
banco_intencao %>% 
  filter(candidato %in% c("Wladimir Garotinho", "Caio Vianna", "Rafael Diniz", "Dr. Bruno Calil", "Professora Natália", "Brancos/Nulos")) %>% 
  ggplot(aes(x = data))+
  geom_line(aes(y = percentual, group = candidato, col = candidato), stat = "identity", size = 1.5)+
  geom_point(aes(y = percentual, group = candidato, col = candidato), size = 3, stat = "identity", size = 1.5)+
  geom_errorbar(aes(ymin = percentual - p_erro, ymax = percentual + p_erro, col = candidato), position=position_dodge(0), size = 1, width = 2)+
  scale_y_continuous(labels=function(x) paste0(x,"%"))+
  scale_x_discrete(breaks = c('29/09/2020', '29/10/2020', '08/11/2020'))+
  #scale_x_date(breaks = as.Date(c('2020-09-29', '2020-10-29', '2020-11-08')))+
  scale_colour_manual(values = c("bisque", "#014d64" ,"#76c0c1" ,"#01a2d9", "#7ad2f6", "#00887d"))+
  theme_fivethirtyeight()+ 
  ylab("Votos")+
  labs(title = "Intenção de votos - 2020", x = NULL,  col = "")+
  theme(axis.text.y = element_text(size = 11),
        axis.title.y = element_text(size = 11),
        axis.title.x = element_text(size = 10),
        axis.text.x = element_text(size = 11),
...
ggplot(banco_familia)+ 
  aes(x=as.numeric(ano_eleitoral))+ 
  
  geom_rect(aes(xmin = 1988, xmax = 2002,ymin = -Inf, ymax = Inf, fill = "#ffffbf"), alpha = 0.006)+
  geom_rect(aes(xmin = 2008, xmax = 2016,ymin = -Inf, ymax = Inf, fill = "#ffffbf"), alpha = 0.006)+
    geom_rect(aes(xmin = 2020, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "#ffffbf"), alpha = 0.006)+
  
  geom_line(aes(y=percent_votos, group = apoiador, col = apoiador), stat = "identity", size = 1.2, alpha = 0.7)+
  labs(col = "")+
 #   geom_point(aes(y=percent_votos, grpup = line_x, col = line_x), stat = "identity", size= 1.5, alpha = 0.7)+
  #labs(col = "")+
  geom_label_repel(aes(y=percent_votos, label = candidato, col= apoiador),
  ...
  geom_vline(xintercept = c(1988, 2002, 2008, 2016, 2020),  size = 2, colour = "#ffffbf", alpha = 0.05 )+
 ...
 # scale_color_manual(values=c("#6794a7", "#01a2d9","#00887d","#a18376","#6794a7", "#01a2d9","#00887d","#a18376"))+
  ...
  ylab("Votos")+ xlab("")+ 
  labs(title = "Histórico eleitoral de Campos dos Goytacazes", col = NULL, fill = "")+
